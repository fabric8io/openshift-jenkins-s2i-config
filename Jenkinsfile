...
dockerTemplate{
    s2iNode{
        ...
        if (env.BRANCH_NAME.startsWith('PR-')) {
            ...
            stage ('build'){
                container('s2i') {
                    sh "s2i build . fabric8/jenkins-openshift-base:${baseImageVersion} ${snapshotImageName} --copy"
                }
            }
            ...
            stage ('s2i build'){
                container('s2i') {
                    sh "s2i build . fabric8/jenkins-openshift-base:${baseImageVersion} fabric8/jenkins-openshift:${newVersion} --copy"
                }
            }
